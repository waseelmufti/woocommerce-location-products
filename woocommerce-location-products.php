...
/* adding custom product field: locations
* This will add multiselect option in the admin product page */ 
function add_custom_product_text_filed(){
	global $woocommerce, $post;
	$_product_locations = get_post_meta( $post->ID, '_locations', true) ? get_post_meta( $post->ID, '_locations', true) : [];
	...
	foreach ($locations_posts as $loc_post) {
		$locations[$loc_post->ID] = $loc_post->post_title;
	}
	
	echo '<div class="options_group">';
	echo '<label for="_locations">Select Locations:</label>';
	$select = '<select id="_locations" name="_locations[]" class="ajax_chosen_select_products" multiple="multiple" data-placeholder="">';
	foreach ($locations as $key => $location) {
		$selected = '';
		if(in_array($key, $_product_locations)){
			$selected = "selected='selected'";
		}
		$select .= '<option value="'.esc_attr($key).'" '.$selected.'>'.$location.'</option>';
	}
	$select .= '</select>';
	echo $select;
	echo '</div>';
	// This comment for single location select field
...
/*
*This function save values of the custom multiselect field
*/ 
function add_custom_product_text_fields_save($post_id){
	
	$_locations = $_POST['_locations'];

	if( !empty($_locations) ){
		update_post_meta( $post_id, '_locations', $_locations );
	}
